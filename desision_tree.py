...
def ContactLensesTest():
    ...
    import copy as cp
    # 加载隐形眼镜相关的 文本文件 数据
    file_name = r'C:\Users\ASUS\Desktop\lenses.csv'
    df = pd.read_csv(file_name,header=None)
    # 解析数据，获得 features 数据
    lenses = df.iloc[:,:]
    lenses = lenses.values.tolist()
    # 得到数据的对应的 Labels
    lensesLabels = ['age', 'prescript', 'astigmatic', 'tearRate']
    #llabels = [example[-1] for example in lenses]
    # 使用上面的创建决策树的代码，构造预测隐形眼镜的决策树
    lensesTree = createTree(lenses, cp.deepcopy(lensesLabels))
    print(lensesTree)
    print(classify(lensesTree,lensesLabels,['pre','hyper','no','reduced']))
